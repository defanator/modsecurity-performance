...
function run {
    ...
    for location in ${locations}; do
		...
		"http://localhost${location}/test.pl?param1=test&para2=test2" 2>&1 >>${PREFIX}${location}.out

            ppid=`cat /var/run/nginx.pid`
            ut=0
            for pid in `pgrep -P ${ppid}`; do
                utime=`cat /proc/${pid}/stat | cut -d ' ' -f 14`
                ut=$((ut + $utime))
            done
            echo "workers_utime: $ut" >>${PREFIX}${location}.out
...
function stats {
    for location in ${locations}; do
        ...
        awk '{if ($1 == "rps:") {print $2}}' ${PREFIX}${location}.out | ministat -n -w 80 | fgrep -v stdin
        echo " latency (ms)"
        awk '{if ($1 == "latency.avg:") {print $2}}' ${PREFIX}${location}.out | ministat -n -w 80 | tail -1
        echo " workers_utime"
        awk '{if ($1 == "workers_utime:") {print $2}}' ${PREFIX}${location}.out | ministat -n -w 80 | tail -1
        echo
    done
}
